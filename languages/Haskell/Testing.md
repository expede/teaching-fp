# Haskell Testing Resources
## General Purpose
- [Tasty](http://documentup.com/feuerbach/tasty)

### Tutorials
- [Official Tasty page](http://documentup.com/feuerbach/tasty)
- [Code testing in Haskell revisited (with Tasty)](http://lambda.jstolarek.com/2014/01/code-testing-in-haskell-revisited-with-tasty/)

## HUnit
- [Repo](https://github.com/pbrisbin/cabal-test-hunit)

### Tutorials
- [Getting started with HUnit](http://leiffrenzel.de/papers/getting-started-with-hunit.html)
- [HUnit 1.0 User's Guide](https://wiki.haskell.org/HUnit_1.0_User's_Guide)

## HSpec
- [HSpec Page](http://hspec.github.io/)

## QuickCheck
- [Repo](https://github.com/nick8325/quickcheck)

### Tutorials
- [A Quick Introduction to QuickCheck Testing](https://www.fpcomplete.com/user/pbv/an-introduction-to-quickcheck-testing)
- [A brief intro to QuickCheck](http://www.stuartgunter.org/intro-to-quickcheck/)

## SmallCheck
- [Repo](https://github.com/feuerbach/smallcheck)
- [SmallCheck and Lazy SmallCheck (whitepaper)](http://www.cs.york.ac.uk/fp/smallcheck/smallcheck.pdf)
